  ...
  switch (toolName) {
    ...
    case 'search_products': {
      try {
        ...
        if (!products.length) return { found: false, message: 'No products found for this search' };
        return {
          found: true,
    ...
    case 'get_product_details': {
      try {
        ...
        return {
          id: p.id, name: p.name, hindi: p.hindi,
          category: p.category, price: p.price,
          original_price: p.original_price, stock: p.stock,
          description: p.description, badge: p.badge, image: p.image,
        };
      } catch (e) {
        return { error: e.message };
      }
    ...
    case 'get_cart': {
      ...
      return {
        items: cartItems.map(i => ({
          ...
          price: i.price, subtotal: i.price * i.qty
        })),
    ...
    case 'get_my_orders': {
      if (!user) return { error: 'Please login to view orders' };
      try {
